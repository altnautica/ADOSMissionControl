// Exempt from 300 LOC soft rule: static demo manifest fixtures.
/**
 * @module mock/first-party-registry
 ...
 * and sizes are cross-checked against the real GitHub release assets. The
 * optical-pod entry is lightly enriched with a top-level icon, screenshots,
 * and MCP tools so the demo exercises every pop-up section. Nothing here
 * reaches an agent; it is display data only.
 *
 * @license GPL-3.0-only
 */

import type { RegistryPluginRow } from "@/lib/plugins/registry-row";
