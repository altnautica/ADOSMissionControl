...
function mk(over: Partial<LocalPluginInstall> = {}): LocalPluginInstall {
  return {
    ...
    name: "Sample Panel",
    halves: ["gcs"],
    gcsContributes: [{ slot: "video.overlay", panelId: "sample-overlay" }],
    grantedCaps: ["telemetry.read"],
    manifestHash: "h1",
    bundle: {
      kind: "archive",
...
describe("local-plugin-installs-store", () => {
  ...
  it("get and remove by plugin+device", () => {
    ...
    expect(
      useLocalPluginInstallsStore
        .getState()
        .get("com.example.sample-panel", "drone-1")?.name,
    ).toBe("Sample Panel");
    useLocalPluginInstallsStore
      .getState()
      .remove("com.example.sample-panel", "drone-1");
    expect(
      useLocalPluginInstallsStore
        .getState()
        .get("com.example.sample-panel", "drone-1"),
    ).toBeUndefined();
  });
