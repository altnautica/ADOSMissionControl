...
- **The parameter download progress bar no longer reads past 100%.** It was
  counting every received value frame rather than every unique parameter, so a
  retried or retransmitted frame over a lossy link double-counted and pushed
  the count past the real total. Downloads on a lossy link also converge more
  patiently now: a second request for an already-downloading board shares that
  download instead of quietly abandoning it, and a stalled retry only gives up
  once it truly stops making progress rather than after a fixed number of
  rounds.
- **A parameter download no longer stalls on a lossy link.** A duplicate or
  retransmitted frame — the same retransmits that caused the earlier
  progress-counter overshoot — was still resetting the download's no-progress
  timer, so a trickle of duplicate traffic kept the missing-index retry from
  ever firing. The timer now only resets when a previously-unseen parameter
  index actually lands, mirroring the duplicate handling already applied to
  the progress display.
- **A malformed parameter frame can no longer complete a download
  instantly.** A stray frame reporting an index of 65535 against a count of
  1 set the expected total to 1 and immediately satisfied it, discarding
  everything already collected, because nothing in the frame format ties a
  response to its request. A real indexed parameter always satisfies
  0 <= index < count; frames failing that check are still cached and still
  resolve a targeted single-parameter read, but can't feed or complete a bulk
  download, and the shown total can't blip downward mid-download.
- **A drone reached through a ground station's radio relay now shows its
  real identity, liveness and parameter stream.** Four root-caused GCS-side
  bugs affected a WFB-linked drone: the Parameters tab stuck at 0/0 until
  a page reload (an unbounded `mintWsTicket` fetch wedged the retry loop,
  now bounded by a 5-second timeout); a slow param download that
  undercounted (compounding client-side timing and state bugs in the
  MAVLink param adapters, fixed by raising the no-progress cap, resending
  `PARAM_REQUEST_LIST` on empty replies, nulling an orphaned download on
  mid-download reconnect, and re-downloading against a new adapter on a
  protocol swap); a fleet badge that flipped from RELAYED to STALE during
  a heavy download (presence freshness now advances on every raw chunk via
  `linkInfo[].lastByteAt`, not only on the radio beacon); and a relayed
  node misidentified as a bare FLIGHT CONTROLLER, fixed by splitting a
  new `agentIdentityKnown` boolean out of `cloudDeviceId` across the node
  presence, merge, enrollment and surface layers. Also admits the RFC 6598
  carrier-grade NAT range (`100.64.0.0/10`, used by Tailscale) to the
  LAN-pair SSRF allowlist so an operator can pair a node by its Tailscale IP.
...
- Pairs with ADOS Drone Agent 0.99.251.
